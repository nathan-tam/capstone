# EVPN Batch Mobility Measurement: Topology and Routing Guide

This guide explains what the `bgp_evpn_capstone_move` test builds, how routing is intended to work, and what to check when behavior is unexpected.
...
`build_topo()` in `test_evpn_capstone_move.py` creates 2 spines, 7 VTEPs, and 7 hosts. Every VTEP connects to both spines. Each host connects to exactly one VTEP (1:1 with 7 hosts and 7 VTEPs).
...
Key constants in `test_evpn_capstone_move.py` (most can be overridden via environment variables):

| Constant | Default | Env var | Meaning |
|----------|---------|---------|----------|
| `NUM_POOL_VMS` | `40` | `NUM_POOL_VMS` | Fixed pool of mobile VMs deployed per measurement iteration |
| `CONTROLLER_VTEPS` | `{"vtep1"}` | — | Hub VTEPs; controller endpoint lives here |
| `CONVERGENCE_WAIT_SECONDS` | `5` | `CONVERGENCE_WAIT_SECONDS` | Post-move wait for BGP convergence before stopping pcap |
| `INITIAL_CONVERGENCE_SECONDS` | `15` | `INITIAL_CONVERGENCE_SECONDS` | Post-deployment wait for initial BGP convergence |
| `SIMULATION_SEED` | random | `SIMULATION_SEED` | RNG seed for reproducibility; logged at the start of each run |
| `SWEEP_BATCH_SIZES` | `1,5,10,20,40` | `SWEEP_BATCH_SIZES` | Comma-separated batch sizes for `test_batch_scaling_sweep` |

### Movement model

The simulation uses a **batch-based measurement** approach:

1. Deploy `NUM_POOL_VMS` MACVLANs once across mobility-eligible hosts.
2. Wait `INITIAL_CONVERGENCE_SECONDS` for all type-2 EVPN routes to settle.
3. Start pcap capture (so only move-related UPDATEs are measured).
4. Move a batch of VMs via `bridge fdb replace` on destination VTEPs
   and `bridge fdb del` on source VTEPs.  This triggers MAC Mobility
   (RFC 7432) — each destination VTEP advertises the claimed MAC with
   an incremented sequence number.
5. Wait `CONVERGENCE_WAIT_SECONDS` for BGP to propagate all UPDATEs.
6. Stop pcap; count MP_REACH / MP_UNREACH.
7. Clean up all VMs and FDB entries.

Destination VTEPs are chosen **uniformly at random** from all mobility-eligible
VTEPs other than the VM's current location.

### Why FDB-based movement?

The previous MACVLAN create/delete approach required 8+ subprocess calls per
move (4 to create, 1 overlap sleep, 4 to delete).  FDB manipulation requires
only 2 commands per move: one `bridge fdb del` on the old VTEP and one
`bridge fdb replace` on the new VTEP.  Commands are batched per-VTEP into
compound shell calls, so the total subprocess count equals the number of
distinct VTEPs involved (typically 6), regardless of batch size.

### MRAI note

This test uses the FRR datacenter default MRAI (`advertisement-interval 0`),
meaning every MAC move triggers an immediate BGP UPDATE.  With MRAI=0 each
move produces ~2 MP_REACH on the spine (one from the new VTEP, one relayed
from the hub), so the relationship is expected to be roughly linear.

To experiment with non-zero MRAI and observe UPDATE batching effects, add
`neighbor TRANSIT_OVERLAY advertisement-interval <seconds>` to each
evpn.conf file.
...
From `test_evpn_capstone_move.py`:

- `CONTROLLER_VTEPS = {"vtep1"}` (hub/controller side is fixed)
- `NUM_POOL_VMS = 40` (fixed pool, deployed once per iteration)
- MRAI uses FRR datacenter default (`advertisement-interval 0`)
- migration behavior is controlled by:
  - `CONVERGENCE_WAIT_SECONDS` (default `5`)
  - `INITIAL_CONVERGENCE_SECONDS` (default `15`)
  - `SWEEP_BATCH_SIZES` (default `1,5,10,20,40`)
  - `SIMULATION_SEED` (random; logged for reproducibility)
- movement engine: `bridge fdb replace` / `bridge fdb del` on VTEPs
  (not MACVLAN create/delete on hosts)
- a non-asserting pre-move connectivity check runs via
