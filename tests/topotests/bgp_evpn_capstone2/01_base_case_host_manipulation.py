...
def test_host_movement(tgen):
    ...
    def move_host_from(curr_hostname, target_hostname, targeted_ip, targeted_mac):
        ...
        def run_command_and_expect():
            ...
            print(f"Before movement show evpn mac:\n{output_before['macs'][targeted_mac]}")
            ...
            print(f"After movement show evpn mac:\n{output_after['macs'][targeted_mac]}")
        ...
        sleep(1)
        tgen.gears["host3"].run(f"echo \"===== CHANGE STARTED at {events['movement_triggered']} =====\" >> /tmp/ping_output.log")
        tgen.gears["host3"].run(f"echo \"===== CHANGE STARTED at {events['movement_finished']} =====\" >> /tmp/ping_output.log")
