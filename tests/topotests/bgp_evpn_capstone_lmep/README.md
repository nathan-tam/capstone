...
To simulate realistic data-plane usage, both the symmetrical and LMEP tests contain a traffic verification step where a static controller endpoint (located on `host1`) attempts to ping recently migrated mobile VMs. 
...
- **Terminal Output**: When running the test (`sudo -E pytest -s tests/topotests/bgp_evpn_capstone_lmep/test_evpn_capstone_lmep.py`), you will see the `[Background] Ping vmX (192.168.100.X) ... OK` messages interleave asynchronously with the "Migrating batch Y" messages.
